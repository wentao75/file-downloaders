...
export class OllamaCaptchaService {
    private readonly OLLAMA_API_URL: string;
    private readonly OLLAMA_MODEL: string;

    constructor() {
        this.OLLAMA_API_URL = process.env.OLLAMA_API_URL || 'http://localhost:11434';
        this.OLLAMA_MODEL = process.env.OLLAMA_CAPTCHA_MODEL || process.env.OLLAMA_MODEL || 'llama3.2-vision:latest';
        console.log('使用验证码识别模型:', this.OLLAMA_MODEL);
    }
    ...
    async init() {
        ...
        console.log('OllamaCaptchaService 初始化完成');
    }
    ...
    private async recognizeWithOllama(base64Image: string): Promise<string> {
        const startTime = Date.now();
        try {
            console.log('开始使用 Ollama 识别验证码...');
...
图片内容：[图片数据已省略]`;
            ...
            const response = await axios.post(`${this.OLLAMA_API_URL}/api/generate`, {
            ...
            console.log(`Ollama 响应耗时: ${duration}ms`);
            ...
            const endTime = Date.now();
            const duration = endTime - startTime;
            console.error(`Ollama API 调用失败 (耗时: ${duration}ms):`, error);
            throw error;
        }
    }
    ...
    async solveCaptcha(page: Page, selector: string = '#captcha-image'): Promise<string> {
        ...
        try {
            ...
            // 使用 Ollama 识别验证码
            const result = await this.recognizeWithOllama(base64Image);
            const endTime = Date.now();
            const duration = endTime - startTime;
